        ...
        <div className="space-y-4 mb-8">
          {['스팸 / 홍보성', '욕설 / 비방 / 혐오 표현', '개인정보 노출', '허위 정보 / 선동', '기타 사유'].map(r => (
            <label key={r} className="flex items-center gap-3 p-3 rounded-lg border border-border bg-bg-base/50 hover:border-primary transition-all cursor-pointer group">
              ...
              />
              <span className={cn("text-sm transition-colors", reason === r ? "text-primary font-bold" : "text-text-secondary group-hover:text-text-primary")}>
                {r}
              </span>
            </label>
          ))}
...
export const PostDetailPage = () => {
  ...
  const isPostAuthor = post.isMy || (post.author !== '익명' && post.author === user?.name);
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-20">
        ...
        <header className="space-y-6">
          <div className="flex flex-wrap items-center gap-3">
             {post.isResolved ? (
              ...
              <span className="px-3 py-1 rounded-full bg-bg-elevated text-text-muted text-[11px] font-bold border border-border tracking-wider">{post.department}</span>
          </div>
          ...
          <div className="flex items-center justify-between py-4 border-y border-border/50">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-bold text-text-primary">{post.isAnonymous ? '익명' : post.author}</p>
                <p className="text-xs text-text-muted">{new Date(post.createdAt).toLocaleString([], { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })}</p>
              </div>
            </div>
            <div className="flex items-center gap-1">
              <button className="p-2 text-text-muted hover:text-danger transition-colors"><Flag size={18} onClick={() => setReportModalOpen(true)} /></button>
            </div>
          </div>
        </header>
      ...
      <section className="space-y-6">
        <h3 className="text-xl font-bold flex items-center gap-2">
          ...
          답변 <span className="text-primary font-black ml-1">{comments.length}</span>
        </h3>
                  ...
                  <div className="flex items-start justify-between mb-4">
                     <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-text-muted">{new Date(comment.createdAt).toLocaleString([], { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1">
                      {isPostAuthor && !post.isResolved && (
                        <button className="text-xs font-bold px-3 py-1.5 rounded-lg bg-bg-surface border border-success/30 text-success hover:bg-success hover:text-white transition-all">채택하기</button>
                      )}
                      <button className="p-2 text-text-muted hover:text-text-primary"><Flag size={14} onClick={() => setReportModalOpen(true)} /></button>
                    </div>
                  </div>
